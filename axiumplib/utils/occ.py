...
from OCC.Core.TopoDS import TopoDS_Shell, TopoDS_Face, TopoDS_Shape
...
from axiumplib.glob_params import FUSE_TOL, FIX_PRECISION
from math import pi


def to_TColgp_Array1OfPnt(pts):
...
def to_TColgp_HArray1OfPnt(pts):
...
def to_TColgp_Array2OfPnt(uv_pts):
...
def solid_from_compound(compound):
...
def fix_solid(solid):
    ...
    fixer.SetPrecision(FIX_PRECISION)
    fixer.Perform()
    return fixer.Solid()
...
def get_wires_from_shape(shape: TopoDS_Shape):
...
def get_elem_from_shape(shape: TopoDS_Shape, elemAbs):
    elems = []
...
def translate(object, vector, copy=False):
...
def rotation_array(object, n, angle=2 * pi, point=[0, 0, 0], axis=[1, 0, 0]):
...
def save_shape_to_brep(shape, filename: str):
...
def save_shape_to_step(shape, filename: str):
