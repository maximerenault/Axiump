...
from axiumplib import BladeBuilder, BladeParameters
import numpy as np
...
def create_bsplinesurface_from_blade(surface_func, num_pts_u=100, num_pts_v=30):
    x_func = lambda x: x**3 * (6 * x**2 - 15 * x + 10)
    uv_points = get_uv_points(surface_func, x_func(np.linspace(0, 1, num_pts_u)), x_func(np.linspace(0, 1, num_pts_v)))
